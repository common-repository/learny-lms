...
class Instructor extends BaseController
{
    ...
    {
        add_action('admin_post_' . self::$plugin_id . '_instructor', array($this, 'post'));
    ...
    public function handle_posts($task)
    {
        switch ($task) {
            ...
            case 'edit_instructor':
                $this->edit_instructor();
                break;
            case 'delete':
                $this->delete_instructor();
                break;
            case 'toggle_status':
                $this->toggle_status();
                break;
    ...
    {
        if (self::verify_nonce('add_instructor_nonce') == true) {
            ...
            $userdata = array(
                ...
                'last_name'     =>  $last_name,
                'role'          =>  self::$custom_roles['instructor']['role'],
                'user_pass'     =>  $password,
                'description'   =>  $bio,
            );
            ...
            if (!$user_id && false == email_exists($email)) {
                ...
                if (is_wp_error($user_id)) {
                    echo json_encode(['status' => false, 'message' => esc_html__("An error occurred while creating the instructor", BaseController::$text_domain)]);
                    return;
                }
            } else {
            ...
            echo json_encode(['status' => true, 'message' => esc_html__("Instructor Added Successfully", BaseController::$text_domain)]);
     ...
     * @return json
     */
    public static function edit_instructor()
    {
        if (self::verify_nonce('edit_instructor_nonce') == true) {

            echo json_encode(['status' => true, 'message' => esc_html__("Instructor Updated Successfully", BaseController::$text_domain)]);
        }
    }
     ...
     * @return json
     */
    public static function delete_instructor()
    {
        if (self::verify_nonce('confirmation_form_nonce') == true) {
            $instructor_id = sanitize_text_field($_POST['id']);
            wp_delete_user($instructor_id);

            echo json_encode(['status' => true, 'message' => esc_html__("Instructor Deleted Successfully", BaseController::$text_domain)]);
        }
    }

    /**
     * TOGGLING INSTRUCTOR STATUS
     *
     * @return json
     */
    public static function toggle_status()
    {
        if (self::verify_nonce('confirmation_form_nonce') == true) {
            $instructor_id = sanitize_text_field($_POST['id']);
            $previous_user_status = get_user_meta($instructor_id, 'ly_status', true);

            $status_update_to = $previous_user_status == 1 ? 0 : 1;
            self::update_user_metas($instructor_id, ['ly_status' => $status_update_to]);
     ...
     * @return array
     */
    public static function get_instructor_course_ids($instructor_id = "")
    {
        ...
        $args = array('post_type' => 'learny-courses', 'author' => $instructor_id);
        $loop = new WP_Query($args);
        while ($loop->have_posts()) : $loop->the_post();
            if (!in_array(get_the_ID(), $course_ids)) {
                array_push($course_ids, get_the_ID());
            }
        endwhile;
    ...
    public static function get_total_enrolment_of_instructor($instructor_id = "")
    {
        if (empty($instructor_id)) {
            $instructor_id = get_current_user_id();
        }
        ...
        foreach ($course_ids as $course_id) {
            $total_enrolment = $total_enrolment + count(Enrolment::get_number_of_enrolled_student($course_id));
        }
